...
choise_char_class → chooseCharClass
else убраны там, где после if идёт return.
rand.Seed добавлен в main(), чтобы числа действительно были случайными.
randint исправлен: теперь верхнее значение включается корректно.
Стиль кода: отступы 4 пробела, пробелы после запятой, строки <= 120 символов*/
